...
@dp.message_handler(Text(equals='Менеджер'), state=SystemState.UserDelete)
async def delete_start(message: types.Message):
    ...
    await SystemState.UserDeleteManager_Q1.set()


@dp.message_handler(IsPhone(), state=SystemState.UserDeleteManager_Q1)
async def search_by_number(message: types.Message, state=FSMContext):
    ...
    if search_phone_to_db:
        ...
        await SystemState.UserDeleteManager_Q2.set()
    else:
...
@dp.message_handler(state=SystemState.UserDeleteManager_Q2)
async def delete_manager_to_db(message: types.Message, state=FSMContext):
    values = await state.get_data()
    del_phone = values['var_delete_phone']
    if message.text.strip().lower() == 'да':
        SqlQuery().delete_row(table_name='managers',
